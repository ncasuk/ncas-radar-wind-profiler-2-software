...
def make_netcdf_snr_winds(trw_files, metadata_file = None, ncfile_location = '.', verbose = True, options = ''):
    ...
    if options == '':
        create_netcdf.main('ncas-radar-wind-profiler-2', date = file_date, dimension_lengths = {'time':len(all_data['time']), 'altitude': all_data['altitude'].shape[1]}, loc = 'land', products = ['snr-winds'], file_location = ncfile_location)
    else:
        create_netcdf.main('ncas-radar-wind-profiler-2', date = file_date, dimension_lengths = {'time':len(all_data['time']), 'altitude': all_data['altitude'].shape[1]}, loc = 'land', products = ['snr-winds'], file_location = ncfile_location, options = options[:-1])
    ...
    remove_empty_variables.main(f'{ncfile_location}/ncas-radar-wind-profiler-2_cao_{file_date}_snr-winds_{options}v1.0.nc', verbose = verbose, skip_check = True)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description = 'Create AMOF-compliant netCDF file for ncas-radar-wind-profiler-2 instrument.')
